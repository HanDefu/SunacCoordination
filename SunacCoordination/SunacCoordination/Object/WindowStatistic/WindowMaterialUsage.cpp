...
E_WindowAluminumType  CWindowMaterialUsage::GetSpecialWinType(E_WindowAluminumType winType, CString p_sAlName)const  //p_sAlName铝型材名称
{
	if (winType==E_WindowAluminum_WM && p_sAlName.CompareNoCase(_T("内开扇"))==0)
	{
...
void CWindowMaterialUsage::ExprotAlInfo(Excel::CExcelUtil& p_excel)//输出型材数据
{
	...
	//根据原型编号和型材系列获取数据库中的数据，若查询结果为空，则只根据原型编号来获取数据库中的数据
	...
	for (UINT i=0; i<alFormulas.size(); i++, nRow++)
	{
		...
		if (_T("框角码1") == alFormulas[i].m_name || _T("扇角码3") == alFormulas[i].m_name || _T("扇角码4") == alFormulas[i].m_name)
		{
			p_excel.SetCellValue(nRow, 3, _T("角码"));
		}
		...
		if (_T("扇料") == alFormulas[i].m_aluminumClassify && m_winAtt.GetMainPrototypeCode().Find(_T("TC")) >= 0)
		{
		...
		if (_T("扇料") == alFormulas[i].m_aluminumClassify && m_winAtt.GetMainPrototypeCode().Find(_T("TLM")) >= 0)
		{
			m_shanliaoTotalLength += length*alFormulas[i].m_nCount;
		}
	}
...
void CWindowMaterialUsageTC::ExportFuliaoInfo(Excel::CExcelUtil& p_excel) //输出辅料数据
{
	...
	{
		p_excel.SetCellValue(66, 3, _T("密封毛条"));
	}
