...
{
	::SetClassLongPtr(m_hWnd,GCLP_HCURSOR,(long)mhCrossCursor);
}
...
Acad::ErrorStatus CGsPreviewCtrl::InitDrawingControl(AcDbDatabase *p_pDb, const TCHAR *space)
{
	// 检测是否设置有效图形数据库
	if (p_pDb == NULL)
	...
	Acad::ErrorStatus es = p_pDb->getBlockTable(pTab, AcDb::kForRead);
	if (es == Acad::eOk)
	{
		// 获得指定的块表记录（一般都是模型空间）
		...
		if (es == Acad::eOk)
		{
			DrawBlockTableRecord(pRec, p_pDb);
...
Acad::ErrorStatus CGsPreviewCtrl::DrawBlockTableRecord(AcDbBlockTableRecord *pRec, AcDbDatabase *p_pDb)
{
	// 初始化控件资源
	Init(theArxDLL.ModuleResourceInstance(), true);

	// 计算图形数据库模型空间的大小
	AcDbExtents ext = GetModelSpaceExtent(p_pDb);
...
{
	CLogDetailDlg dlg;
	dlg.SetDwgFileName(msFileName);
	dlg.SetDwgFileDir(msFileName);
	dlg.DoModal();
}
*/
