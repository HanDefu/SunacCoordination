...
bool WebIO::DownloadJPGFile(const int fileId)
{
	_ns1__CadImgDownload ns;
	ns.Id = fileId;
	_ns1__CadImgDownloadResponse nsResponse;

	ArgumentSettingServiceSoapProxy cadWeb;
	InitSoapTime(cadWeb);
	int nRet = cadWeb.CadImgDownload(&ns, nsResponse);
	
	if (nsResponse.CadImgDownloadResult == NULL)
	{
		return false;
	}
	...
	xml.SetDoc((*(nsResponse.CadImgDownloadResult)).c_str());

	return true;
}

bool WebIO::DownloadDWGFile(const int fileId)
{
	return true;
}

bool WebIO::DownloadFile(CString &strFileURLInServer, CString &strFileLocalFullPath) const
{
